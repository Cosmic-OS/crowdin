...
def push_as_commit(base_path, path, name, branch, username):
    ...
    try:
        repo.git.commit(m='Cosmic-OS Automatic translation import')
    except:
    ...
    # Push commit
    try:
        repo.git.push('git@github.com:Cosmic-OS/%s' % (name),
                      'HEAD:%s' % branch)
...
def main():
    ...
    base_path = os.getenv('COSMIC_CROWDIN_BASE_PATH')
    if base_path is None:
        cwd = os.getcwd()
        print('You have not set COSMIC_CROWDIN_BASE_PATH. Defaulting to %s' % cwd)
        base_path = cwd
    else:
        base_path = os.path.join(os.path.realpath(base_path))
    if not os.path.isdir(base_path):
        print('COSMIC_CROWDIN_BASE_PATH + branch is not a real directory: c'
              % base_path)
        sys.exit(1)
